...
def inferno(itens_disponiveis, historico_compras):
    usuario = login() 
...
def main():
    ...
    if escolha.lower() == 's':
        ...
        segunda_escolha = input("Gostaria de fazer cadastro? (s/n): ")
        if segunda_escolha.lower() == "s":
            cadastro()
            usuario = login()
        elif segunda_escolha.lower() == "n":
            while True:
              inferno(itens_disponiveis, historico_compras)
    else:
        adicionar_usuario()
        while True:
            usuario = login()
